---
name: code-optmz-graph
description: "Build knowledge graph from any folder. Trigger: /code-optmz-graph"
trigger: /code-optmz-graph
---
...
```
/code-optmz-graph                                             # full pipeline on current directory → Obsidian vault
/code-optmz-graph <path>                                      # full pipeline on specific path
/code-optmz-graph https://github.com/<owner>/<repo>           # clone repo then run full pipeline on it
/code-optmz-graph https://github.com/<owner>/<repo> --branch <branch>  # clone a specific branch
/code-optmz-graph <url1> <url2> ...                           # clone multiple repos, build each, merge into one cross-repo graph
/code-optmz-graph <path> --mode deep                          # thorough extraction, richer INFERRED edges
/code-optmz-graph <path> --update                             # incremental - re-extract only new/changed files
/code-optmz-graph <path> --directed                            # build directed graph (preserves edge direction: source→target)
/code-optmz-graph <path> --whisper-model medium                # use a larger Whisper model for better transcription accuracy
/code-optmz-graph <path> --cluster-only                       # rerun clustering on existing graph
/code-optmz-graph <path> --no-viz                             # skip visualization, just report + JSON
/code-optmz-graph <path> --html                               # (HTML is generated by default - this flag is a no-op)
/code-optmz-graph <path> --svg                                # also export graph.svg (embeds in Notion, GitHub)
/code-optmz-graph <path> --graphml                            # export graph.graphml (Gephi, yEd)
/code-optmz-graph <path> --neo4j                              # generate graphify-out/cypher.txt for Neo4j
/code-optmz-graph <path> --neo4j-push bolt://localhost:7687   # push directly to Neo4j
/code-optmz-graph <path> --mcp                                # start MCP stdio server for agent access
/code-optmz-graph <path> --watch                              # watch folder, auto-rebuild on code changes (no LLM needed)
/code-optmz-graph <path> --wiki                               # build agent-crawlable wiki (index.md + one article per community)
/code-optmz-graph <path> --obsidian --obsidian-dir ~/vaults/my-project  # write vault to custom path (e.g. existing vault)
/code-optmz-graph add <url>                                   # fetch URL, save to ./raw, update graph
/code-optmz-graph add <url> --author "Name"                   # tag who wrote it
/code-optmz-graph add <url> --contributor "Name"              # tag who added it to the corpus
/code-optmz-graph query "<question>"                          # BFS traversal - broad context
/code-optmz-graph query "<question>" --dfs                    # DFS - trace a specific path
/code-optmz-graph query "<question>" --budget 1500            # cap answer at N tokens
/code-optmz-graph path "AuthModule" "Database"                # shortest path between two concepts
/code-optmz-graph explain "SwinTransformer"                   # plain-language explanation of a node
```
